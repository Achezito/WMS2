...
// Función para obtener los números de serie existentes en el inventario
function obtenerNumerosSerieExistentes($edificio_id)
{
    $conn = Conexion::get_connection();
    $query = "SELECT serie FROM inventario WHERE edificio_id = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("i", $edificio_id);
    $stmt->execute();
    $result = $stmt->get_result();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Asignar proveedor directamente sin opción de agregar nuevo
    $proveedor_id = $_POST['proveedor'];

    // Almacena los números de serie que se intentan insertar
    $series = $_POST['series'];
    $modelos = $_POST['modelos'];
    $estatus_id = 1;  // 'Disponible'

    // Verificar si los números de serie ya existen en el inventario
    $numeros_serie_existentes = obtenerNumerosSerieExistentes($edificio_id);
    $numeros_serie_intentos = [];

    foreach ($series as $serie) {
        $numeros_serie_intentos[] = $serie;
    }

    // Comprobar si hay duplicados
    $duplicados = array_intersect($numeros_serie_existentes, $numeros_serie_intentos);

    if (!empty($duplicados)) {
        // Si hay duplicados, devolver un JSON con el mensaje de error
        echo json_encode(['success' => false, 'message' => 'Error: No se logró la transacción porque los siguientes números de serie ya existen en el inventario: ' . implode(", ", $duplicados)]);
        exit();
    } else {
        // Si no hay duplicados, proceder con las inserciones
        ...
        echo json_encode(['success' => true, 'message' => 'La transacción se realizó con éxito.']);
        exit();
    }
}
            ...
            <section class="content">
                <div class="form-container">
                    ...
                    <form method="POST" action="">
                        <input type="hidden" name="tipos[]" id="tipos" value="">
                        ...
                        <div id="entradaMateriales">
                            ...
                            <div class="form-group">
                                ...
                                <select name="tipos[]" id="tipo_material">
