    ...
    if (!empty($historial)) {
        ...
        foreach ($historial as $registro) {
            ...
            if (!$ocultarBoton) {
                ...
                echo "<td>
                        <button class='ver-mas-btn' data-tipo-operacion='$tipoOperacion' data-operacion-id='$operacionId' data-index='$operacionId' onclick='abrirModal(event)'>
                            Ver Más
                        </button>
                    </td>";
            }
        ...
        echo "<p class='p1'>No se encontraron registros para este edificio.</p>";
    }
}
...
function dibujar_historial_mantenimientos($edificio_id) {
    ...
    dibujar_historial('historial_mantenimientos', $columnas, $campos, $edificio_id, null, true);
}
